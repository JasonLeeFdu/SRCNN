...
def prepareTrainingData(directory,recordName):
    ...
    for img_name in fileList:
        ...
        if width > 600 or height > 600: #长边400
            if width > height :
                factor = width / 400
                imgGT = cv.resize(imgGT, (400,int(height/factor)),interpolation=cv.INTER_CUBIC)
                #cv.imwrite(DIR+img_name, imgGT)
                width = imgGT.shape[1]
                height = imgGT.shape[0]
            else:
                factor = height / 400
                imgGT = cv.resize(imgGT, (int(width/factor), 400), interpolation=cv.INTER_CUBIC)
                #cv.imwrite(DIR+img_name, imgGT)
                width = imgGT.shape[1]
                height = imgGT.shape[0]
            ...
            for y in np.arange(0,height-PATCH_SIZE+1,STRIDE):
                ...
                if AUG:
                    ...
                    for subGT, subX in pairList:                     # 居然写反了，怪不得越训越差
                        #cv.imwrite('a.bmp',subGT)
                        #cv.imwrite('b.bmp',subX)
